...
<div class="rnav">
<ul>
	<li><a <?php  echo ($extdisplay=='' ? 'class="current"':'') ?> href="config.php?display=<?php echo urlencode($display)?>"><?php echo _("트렁크 추가")?></a></li>
<?php 
...
if (!$tech && !$extdisplay) {
?>
	<h2><?php echo _("트렁크 추가")?></h2>
<?php
	$baseURL   = $_SERVER['PHP_SELF'].'?display='.urlencode($display).'&';
  $trunks[] = array('url'=> $baseURL.'tech=SIP', 'tlabel' =>  _("SIP 트렁크 추가"));
  //OLSSOO
  ...
  $trunks[] = array('url'=> $baseURL.'tech=CUSTOM', 'tlabel' =>  _("커스텀 트렁크 추가"));
...
<?php 
		...
		$upper_tech = strtoupper($tech);
		echo "<h2>".sprintf(_("%s 트렁크 추가"),$upper_tech).($upper_tech == 'ZAP' && ast_with_dahdi()?" ("._("DAHDi compatibility mode").")":"")."</h2>";
	} 
