...
<div class="rnav">
<ul id="routelist">
	<li><a <?php  echo ($extdisplay=='' ? 'class="current"':'') ?> href="config.php?display=<?php echo urlencode($display)?>"><?php echo _("라우트 추가")?></a></li>
<?php 
...
if ($extdisplay) { // editing
	$tlabel = sprintf(_("%s 라우트 삭제"),$route_info['name']);
...
?>
	<form enctype="multipart/form-data" autocomplete="off" id="routeEdit" name="routeEdit" action="config.php" method="POST" onsubmit="return routeEdit_onsubmit('<?php echo ($extdisplay != '' ? "editroute" : "addroute") ?>');">
		...
		<table>
    <tr>
      <td colspan="2"><h5><?php echo _("라우트 설정") ?><hr></h5></td>
    </tr>
...
?>
    <tr>
      <td colspan="2"><h5><?php echo _("추가 설정") ?><hr></h5></td>
    </tr>
<?php
...
?>
    ...
    <tr><td colspan="2">
      <input type="button" id="dial-pattern-add"  value="<?php echo _("+ 다이얼플랜 필드 추가")?>" />
    </td></tr>
<?php
...
<?php if ($extdisplay != '') { // editing ?>
		<tr>
			<td colspan="2">
				<input type="submit" value="<?php echo _("트렁크 추가")?>">
			</td>
		</tr>
<?php } // if $extdisplay ?>
...
?>
        ...
        <h6>
          ...
          <input name="copyroute" type="submit" value="<?php echo _("동일한 라우트 복사/생성");?>"/>
				</h6>
			</td>
		</tr>
		</table>
