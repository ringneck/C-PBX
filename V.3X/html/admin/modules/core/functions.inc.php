...
function core_devices_configpageinit($dispnum) {
  ...
  // $amp_conf['DEVICE_PICKUPGROUP']
    ...
    $tt = _("How to dial this device. This will be device specific. For example, a custom device which is really a remote SIP URI might be configured such as SIP/joe@somedomain.com");
		...
		$currentcomponent->addoptlistitem('devicelist', 'sip_generic', _("SIP내선"));
		...
		$currentcomponent->addoptlistitem('devicelist', 'custom_custom', _("커스텀내선"));
		}
		if ( $dispnum != 'devices' ) {
			$currentcomponent->addoptlistitem('devicelist', 'virtual', _("None(가상내선)"));
		}
	...
	if ( $action == 'del' ) { // Deleted
		...
		if ( $display != 'extensions' )
			...
			if ( $display != 'extensions') {
				...
				$currentcomponent->addguielem('_top', new gui_pageheading('title', _("내선번호 추가")), 0);
			}
			$currentcomponent->addguielem('_top', new gui_label('instructions', _("추가할 내선의 종류를 아래의 목록에서 선택하세요")));
			$currentcomponent->addguielem('내선', new gui_selectbox('tech_hardware', $currentcomponent->getoptlist('devicelist'), '', _("내선종류"), '', false));
