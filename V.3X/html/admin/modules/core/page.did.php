...
<div class="rnav">
<ul>
	<li><a <?php echo ($extdisplay=='' ? 'class="current"':'') ?> href="<?php echo $display_add?>"><?php echo _("수신 설정 추가")?></a></li>
...
<?php 
    if ($extdisplay) {	
      ...
      if (is_array($ininfo) && !empty($ininfo)) {
        ...
        $delete_url = true;
?>
		<h2><?php echo _("수신 설정")?>: <?php echo !empty($description)?$description:$extdisplay; ?></h2>
<?php
      } else {
...
?>
		...
		<tr>
			<td><a href="#" class="info"><?php echo _("발신자 이름 프리픽스")?><span><?php echo _('You can optionally prefix the CallerID name. ie: If you prefix with "Sales:", a call from John Doe would display as "Sales:John Doe" on the extensions that ring.')?></span></a>:</td>
...
<?php } ?>
		<tr>
			<td><a href="#" class="info"><?php echo _("Ringing 추가")?><span><?php echo _('Some devices or providers require RINGING to be sent before ANSWER. You\'ll notice this happening if you can send calls directly to a phone, but if you send it to an IVR, it won\'t connect the call.')?></span></a>:</td>
...
?>
		<tr><td colspan="2"><h5><?php echo _("실행")?><hr></h5></td></tr>
...
?>
		<tr>
			<td colspan="2">
				<h6><input name="Submit" type="submit" value="<?php echo _("변경저장")?>" tabindex="<?php echo ++$tabindex;?>">&nbsp;&nbsp;
				<input name="submitclear" type="submit" value="<?php echo _("실행 삭제하고 변경저장")?>" ></h6>
			</td>		
		</tr>
		</table>
