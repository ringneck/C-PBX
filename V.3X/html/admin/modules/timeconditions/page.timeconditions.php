...
<div class="rnav"><ul id="timelist">
    <li><a id="<?php echo ($itemid=='' ? 'current':'') ?>" href="config.php?display=<?php echo urlencode($dispnum)?>"><?php echo _("시간 라우트 추가")?></a></li>
<?php
...
?>
	<h2><?php echo ($itemid ? _("시간 라우트:")." ". $itemid : _("시간 라우트 추가")); ?></h2>
<?php
	if ($itemid){
		...
		$tlabel = sprintf(_("시간 라우트 삭제: %s"),trim($thisItem['displayname']) == '' ? $code : $thisItem['displayname']." ($code) ");
...
?>
	...
	<table>
	<tr><td colspan="2"><h5><?php echo ($itemid ? _("시간 라우트 수정") : _("시간 라우트 추가")) ?><hr></h5></td></tr>
...
<?php		}?>
	...
	<tr>
		<td><a href="#" class="info"><?php echo _("시간 라우트 이름:")?><span><?php echo _("Give this Time Condition a brief name to help you identify it.")?></span></a></td>
...
<?php } ?>
	<tr>
		<td><a href="#" class="info"><?php echo _("시간 그룹:")?><span><?php echo _("Select a Time Group created under Time Groups. Matching times will be sent to matching destination. If no group is selected, call will always go to no-match destination.")?></span></a></td>
...
?>
	<tr><td colspan="2"><br><h5><?php echo _("시간 조건이 맞을 경우 라우트 (O)")?>:<hr></h5></td></tr>
<?php
//draw goto selects
if (isset($thisItem)) {
...
?>
	...
	<tr><td colspan="2"><br><h5><?php echo _("시간 조건이 안 맞을 경우 라우트 (X)")?>:<hr></h5></td></tr>
...
?>
	...
	<tr>
		<td colspan="2"><br><h6><input name="Submit" type="submit" value="<?php echo _("변경저장")?>" tabindex="<?php echo ++$tabindex;?>"></h6></td>
	</tr>
	</table>
