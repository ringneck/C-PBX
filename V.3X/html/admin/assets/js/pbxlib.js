...
+'Please wait...'
+'</progress>').dialog({title:'Reloading...',resizable:false,modal:true,height:50,position:['center',50],close:function(e){$(e.target).dialog("destroy").remove();}});$.ajax({type:'POST',url:document.location.pathname,data:"handler=reload",dataType:'json',timeout:760000,success:function(data){box.dialog('destroy').remove();if(!data.status){var r='<h3>'+data.message+'<\/h3>'
