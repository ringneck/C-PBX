...
function reportReportCall($smarty, $module_name, $local_templates_dir, &$pDB_cdr, &$pDB_billing , $arrConf)
{
    ...
    $date_from = isset($date_ini_tmp)?$date_ini_tmp:date("Y m d");
    $date_to   = isset($date_end_tmp)?$date_end_tmp:date("Y m d");
    ...
    $date_to, $_POST, array("date_from" => date("Y M d"),"date_to" => date("Y M d")),true);
...
function createFieldForm(){
    $arrFormElements = array(
            ...
            "date_from" => array( "LABEL"                  => _tr("Start date"),
                                  "REQUIRED"               => "yes",
                                  "INPUT_TYPE"             => "DATE",
                                  "INPUT_EXTRA_PARAM"      => array("FORMAT" => "%Y %m %d"),
...
<html>
<head><title>내선별통계 순위10개</title></head>
<body>
