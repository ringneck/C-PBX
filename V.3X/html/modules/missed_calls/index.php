...
function reportMissedCalls($smarty, $module_name, $local_templates_dir, &$pDB, &$pDBACL, $pACL, $arrConf)
{
    ...
    $date_start = (isset($date_start))?$date_start:date("Y m d").' 00:00';
    $date_end   = (isset($date_end))?$date_end:date("Y m d").' 23:59';
...
function createFieldFilter(){
    ...
    $arrFormElements = array(
	    ...
	    "date_start"  => array("LABEL"                  => _tr("Start Date"),
				    "REQUIRED"               => "yes",
				    "INPUT_TYPE"             => "DATE",
				    "INPUT_EXTRA_PARAM"      => array("TIME" => true, "FORMAT" => "%Y %m %d %H:%M"),
				    "VALIDATION_TYPE"        => "",
				    "VALIDATION_EXTRA_PARAM" => "^[[:digit:]]{4}[[:space:]]+[[:digit:]]{1,2}[[:space:]]+[[:digit:]]{1,2}[[:space:]]+[[:digit:]]{1,2}:[[:digit:]]{1,2}$"),
