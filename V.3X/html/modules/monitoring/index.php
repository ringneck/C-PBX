...
function reportMonitoring($smarty, $module_name, $local_templates_dir, &$pDB, $pACL, $arrConf, $user, $extension, $esAdministrador)
{
    ...
    $date_end = getParameter("date_end"); // OLSSOO

    $path_record = $arrConf['records_dir'];

    $_POST['date_start'] = isset($date_ini)?$date_ini:date("Y m d");
    $_POST['date_end']   = isset($date_end)?$date_end:date("Y m d");  
    
    //OLSSOO echo date("Y m d");
    ...
    //OLSSOO strtitime 함수는 timestamp만 인식해서 필터에 공백제거
    $except_space_date_start = preg_replace("/\s+/", "", $_POST['date_start']);
    $except_space_date_end = preg_replace("/\s+/", "", $_POST['date_end']);

    $date_initial = date('Y-m-d', strtotime($except_space_date_start))." 00:00:00";
    $date_final   = date('Y-m-d', strtotime($except_space_date_end))." 23:59:59";  
    ...
    if($oGrid->isExportAction()){
        ...
        if(is_array($arrResult) && $totalMonitoring>0){
            foreach($arrResult as $key => $value){
                $arrTmp[0] = date('Y m d',strtotime($value['calldate']));
        ...
        if(is_array($arrResult) && $total>0){
            ...
            foreach($arrResult as $key => $value){
                if($esAdministrador)
                    ...
                    $arrTmp[0] = "";
                $arrTmp[1] = date('Y m d',strtotime($value['calldate']));
...
<!DOCTYPE html>
<html>
<head><title>OLSSOO D-PBX</title></head>
<body>
...
function createFieldFilter(){
    ...
    $arrFormElements = array(
            "date_start"  => array(           "LABEL"                  => _tr("Start_Date"),
                                              ...
                                              // OLSSOO "VALIDATION_EXTRA_PARAM" => "^[[:digit:]]{1,2}[[:space:]]+[[:alnum:]]{3}[[:space:]]+[[:digit:]]{4}$"),
                                              "VALIDATION_EXTRA_PARAM" => "^[[:digit:]]{4}[[:space:]]+[[:digit:]]{1,2}[[:space:]]+[[:digit:]]{1,2}$"),
