...
function grafic($du, $totIn, $totOut, $tot, $ext)
{
	...
	if( $tot != 0 )
	{
		...
		$graph->legend->SetFont(FF_NANUMGOTHIC,FS_BOLD,12);
		...
		$graph->title->Set(_tr("Number of calls extension")." $ext");
		...
		$p1->value->SetFont(FF_NANUMGOTHIC,FS_BOLD,12);
		...
		$p1->SetLegends(array(_tr("Incoming Calls")." ".$totIn,
		                     _tr("Outcoming Calls")." ".$totOut));
	...
	else
	{
		...
		$title = new Text(_tr("The extension")." $ext "._tr("does not have calls yet"));
		$title->ParagraphAlign('center');
		//$title->SetFont(FF_FONT2,FS_BOLD);
		$title->SetFont(FF_NANUMGOTHIC,FS_BOLD,12);
		$title->SetMargin(3);
		...
		$t1 = new Text(_tr("No exist calls for this number"));
...
function grafic_queue(&$pDB_ast_cdr, &$pDB_ast, $queue, $dti, $dtf)
{
    ...
    */
	...
	$title = new Text(_tr("No records found"));
	$title->ParagraphAlign('center');
	$title->SetFont(FF_NANUMGOTHIC,FS_BOLD,12);
	//$title->SetFont(FF_FONT2,FS_BOLD);
	$title->SetMargin(3);
	...
	$t1 = new Text(_tr("There are no data to present"));
	$t1->SetBox("white","black",true);
	$t1->ParagraphAlign("center");
	$t1->SetColor("black");
...
function grafic_trunk(&$pDB_ast_cdr, &$pDB_ast, $module_name, $trunk, $dti, $dtf)
{
	...
	if( $tot != 0 )
	{
		...
		$graph->title->Set(_tr("Total Time"));
		$graph->title->SetFont(FF_NANUMGOTHIC,FS_BOLD,12);
		...
		$p1->SetLegends(array( _tr("Incoming Calls").":\n".SecToHHMMSS($arrResult[0]),
...
function grafic_trunk2(&$pDB_ast_cdr, &$pDB_ast, $module_name, $trunk, $dti, $dtf)
{
	...
	if( $tot != 0 )
	{
		...
		$graph->title->Set(_tr("Number of Calls"));
		$graph->title->SetFont(FF_NANUMGOTHIC,FS_BOLD,12);
		//$graph->title->SetFont(FF_VERDANA,FS_BOLD,18);
		$graph->title->SetColor("#444444");
		...
		$p1->SetLegends(array( _tr("Incoming Calls").":  ".$arrResult[0],
		                      _tr("Outcoming Calls").": ".$arrResult[1] ));
	...
	else
	{
		...
		$title = new Text(_tr("Number of Calls"));
		$title->ParagraphAlign('center');
		$title->SetFont(FF_NANUMGOTHIC,FS_BOLD,12);
		//$title->SetFont(FF_FONT2,FS_BOLD);
		$title->SetMargin(3);
