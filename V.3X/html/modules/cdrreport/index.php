...
function _moduleContent(&$smarty, $module_name)
{
    ...
    //OLSSOO $paramFiltro['date_end'], $paramFiltro, array('date_start' => date("d M Y"),'date_end' => date("d M Y")),true);
    $paramFiltro['date_end'], $paramFiltro, array('date_start' => date("Y m d"),'date_end' => date("Y m d")),true);
	...
	$arrResult = $oCDR->listarCDRs($paramFiltro, $limit, $offset);
        ...
        if(is_array($arrResult['cdrs']) && $total>0){
            foreach($arrResult['cdrs'] as $key => $value){
                ...
                $sTiempo = "{$value[8]}초";
                if ($value[8] >= 60) {
                      if ($iDuracion > 0) $sTiempo .= " ({$iDuracion}시 {$iMin}분 {$iSec}s초)";
                      elseif ($iMin > 0)  $sTiempo .= " ({$iMin}분 {$iSec}초)";
                }
                $arrTmp[8] = $sTiempo;
                $arrData[] = $arrTmp;
            }
	...
	$arrResult = $oCDR->listarCDRs($paramFiltro, $limit, $offset);
        ...
        if(is_array($arrResult['cdrs']) && $total>0){
            foreach($arrResult['cdrs'] as $key => $value){
                ...
                $sTiempo = "{$value[8]}초";
                if ($value[8] >= 60) {
                      if ($iDuracion > 0) $sTiempo .= " ({$iDuracion}시 {$iMin}분 {$iSec}초)";
                      elseif ($iMin > 0)  $sTiempo .= " ({$iMin}분 {$iSec}초)";
                }
                $arrTmp[8] = $sTiempo;
                $arrData[] = $arrTmp;
