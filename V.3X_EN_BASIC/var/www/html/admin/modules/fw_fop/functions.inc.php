		...
		case "asterisk":
			...
			if ($result) {
				if ($exit_val != 0) {
					...
					$nt->add_error('freepbx','reload_fop', _('Could not reload FOP server'), $desc);
				...
				$desc = _('An error occured trying to configure FOP. There may be a conflict with another module or there may be permission problems. Check the FreePBX log for more details.');
...
if ($amp_conf['FOPWEBROOT'] != $fopwebroot) {
	freepbx_log(FPBX_LOG_CRITICAL, _("fw_fop is NOT confgured as your Operator Panel, attempting to disable, FOPWEBROOT: " . $amp_conf['FOPWEBROOT'] . "."));
...
if ($fhandle === false) {
	...
	freepbx_log(FPBX_LOG_CRITICAL, _("fw_fop failed to create/overwrite config file $op_conf."));
	return false;
}
