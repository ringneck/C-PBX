...
class core_conf {
	...
	function generate_http_additional($ast_version) {
		...
		$output .= "tlsenable=".($freepbx_conf->get_conf_setting('HTTPSENABLED') ? 'yes' : 'no')."\n";
		$output .= "tlsbindaddr=".$freepbx_conf->get_conf_setting('HTTPSBINDADDRESS')."\n";
		$output .= "tlscertfile=".$freepbx_conf->get_conf_setting('HTTPSCERTFILE')."\n";
		$output .= "tlsprivatekey=".$freepbx_conf->get_conf_setting('HTTPSPRIVATEKEY')."\n";
		return $output;
	}
      ...
      foreach ($routes as $route) {
        ...
        unset($add_extra_pri1);
			...
			*/
			...
			$ext->add($context, $exten, '', new ext_read('INPUT', '', 1, '', '', 1)); // norman 4 => 1
                        $ext->add($context, $exten, '', new ext_setvar('INPUT', '1')); // INPUT <= 1
...
file_put_contents('/tmp/freepbx-debug-sip.txt', print_r($_REQUEST,true). print_r($sipfields, true));
